...
3.  **传递性 (Transitivity):**
    ...
    *   关系矩阵: 如果 $M_R$ 是关系矩阵，那么 $M_R^2$ （布尔矩阵乘法）中所有非零元素对应的位置在 $M_R$ 中也必须是非零（即 $M_R \lor M_R^2 = M_R$）。
...
有时一个关系可能不具备我们想要的性质（如自反性、对称性、传递性），我们希望**尽可能少地添加**一些有序对，使得新关系具有该性质。这个“添加最少元素得到性质”的过程就叫做求**闭包 (Closure)**。
